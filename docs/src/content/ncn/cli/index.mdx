...
Initializes the on-chain `ProgramConfig` singleton. Must be run once per deployment. The signer of `--authority-path` becomes the initial program authority and is recorded as `ProgramConfig.authority`.
