...
|---|---|---|---|
| `--keypair <PATH>` | `-k` | `SVMGOV_KEY` | Signer keypair JSON file or Ledger URI |
...
Pass a Solana Ledger URI to `--keypair` (or `--staker-keypair`):

```bash
svmgov update-global-config \
  --max-support-epochs 10 --discussion-epochs 10 --voting-epochs 4 \
  --keypair 'usb://ledger?key=0/0' \
  --squads "$SQUADS" --rpc-url "$MAINNET_RPC"
```

Use the derivation path for your account: `?key=0/0` selects `m/44'/501'/0'/0'`;
omitting `?key=` uses Solana's default `m/44'/501'`. A device can be selected with
`usb://ledger/<PUBKEY>?key=0/0`, where `<PUBKEY>` is its default-path public key.
Unlock the Ledger, open the Solana app, and enable blind signing for governance
and Squads instructions. Approve each transaction on the device. In Squads mode,
the Ledger signs the proposal creation transaction as a multisig member and fee payer.

Ledger URIs also work in `SVMGOV_KEY` and the existing keypair config values.
Building from source on Debian/Ubuntu additionally requires `libudev-dev`.
...
The default `operator_api_url` is the <a href="/ncn/router">main NCN router</a>.
...
| `--network` | String | No | Network override |
| `--staker-keypair` | String | Yes | Staker keypair JSON file or Ledger URI |
