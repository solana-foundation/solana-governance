...
| `--description` | String | Yes | — | Full-commit-SHA `blob` URL to one `.md` file in `solana-governance-proposals`; byte length must be ≤ configured `max_description_length` and ≤ 500 |
...
| `--description` | String | Yes | Replacement commit-pinned Markdown `blob` URL |
| `--skip-link-check` | flag | No | Skip only the CLI's GitHub/frontmatter preflight |
