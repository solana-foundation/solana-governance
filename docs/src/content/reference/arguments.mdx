...
| `--squads <PUBKEY>` | — | — | Squads vault address for supported transactions; multisig state addresses are also accepted |
| `--squads-vault-index <U8>` | — | — | Squads vault index; defaults to `0` and must match the supplied vault |
...
`--squads` accepts the vault address shown in the Squads app. The CLI looks up its
multisig on the selected RPC and verifies the vault derivation before loading the
signer. Vault lookup requires an RPC that supports `getProgramAccounts`; it can
take several seconds on mainnet. Existing multisig state addresses skip the lookup.
For a sub-account, also supply its index with `--squads-vault-index`.
