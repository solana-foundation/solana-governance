...
| `description` | `String` | Full-commit-SHA GitHub `blob` URL to one `.md` file in `solana-governance-proposals` (length in bytes ≤ `max_description_length`, ≤ 500) |
...
| `snapshot_slot_offset` | `i64` | Slot offset from epoch start for the snapshot | ≤ 414,000 |
...
| `description: String` | `String` | Non-empty, ≤ `max_description_length`, full-commit-SHA GitHub `blob` URL to one `.md` file in `solana-governance-proposals` |
...
Replaces a proposal's pinned document URL. It does not resize the proposal account or alter any lifecycle/voting state.

**Signer:** the proposal's original `author`.

| Parameter | Type | Validation |
|---|---|---|
| `description` | `String` | Same canonical, full-commit-SHA Markdown `blob` URL validation as `create_proposal` |

The instruction is permitted during support while the support window remains open. Once support has activated the lifecycle, it remains permitted only while `clock.epoch < start_epoch`. It is rejected at the start of voting and for finalized proposals. Emits `ProposalDescriptionUpdated`, including the old URL, new URL, author, and timestamp.
...
- Sets `start_epoch` to the epoch after the snapshot and sets `end_epoch` from the voting duration
- Derives `consensus_result` PDA
- CPIs to `ncn-snapshot::init_ballot_box`, with ballot expiry at the first slot of `start_epoch`
...
| `DescriptionInvalid` | Description must be a full-commit-SHA GitHub `blob` URL to one `.md` file in `solana-governance-proposals` |
...
| `UnauthorizedProposalUpdate` | Only the original proposal author can update its document URL |
